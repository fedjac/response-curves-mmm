...
#Set start values, lower bound and upper bound. 
#These values should be based on client's Return data and are not default values
startValVec = c(25000,100,1.5,100000)
minValVec = c(0,0,1.01,0)
maxValVec = c(500000, 500000, 2, 10000000)

#Run paramater optimization function
...
#Ensure for successful convergence by validating that convergence code is set to 0
optim.parms
...
#Function generates expected return levels given specific investments and parameter values
#Function also controls which campaings(categories) are most profitable given a certain investment
adbudgReturn = function(a,b,c,d,Spend){
  adbudgReturn = sum(b+(a-b)*((Spend^c)/(d+(Spend^c))))
  return(adbudgReturn)
}

#ReturnGoal should be higher than the total return from existing campaigns
returnGoal = 600000
increment = 1000
...
#This sections returns recommended investment for each campaign
